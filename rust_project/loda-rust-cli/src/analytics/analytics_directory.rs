...
impl AnalyticsDirectory {
    ...
    pub fn histogram_instruction_unigram_file(&self) -> PathBuf {
        let path = self.analytics_directory.join("histogram_instruction_unigram.csv");
        assert!(path.is_absolute());
        PathBuf::from(path)
    }

    pub fn histogram_instruction_bigram_file(&self) -> PathBuf {
        let path = self.analytics_directory.join("histogram_instruction_bigram.csv");
        assert!(path.is_absolute());
        PathBuf::from(path)
    }

    pub fn histogram_instruction_trigram_file(&self) -> PathBuf {
        let path = self.analytics_directory.join("histogram_instruction_trigram.csv");
        assert!(path.is_absolute());
        PathBuf::from(path)
    }

    pub fn histogram_instruction_skipgram_file(&self) -> PathBuf {
        let path = self.analytics_directory.join("histogram_instruction_skipgram.csv");
        assert!(path.is_absolute());
        PathBuf::from(path)
    }

    pub fn histogram_target_unigram_file(&self) -> PathBuf {
        let path = self.analytics_directory.join("histogram_target_unigram.csv");
        assert!(path.is_absolute());
        PathBuf::from(path)
    }

    pub fn histogram_target_bigram_file(&self) -> PathBuf {
        let path = self.analytics_directory.join("histogram_target_bigram.csv");
        assert!(path.is_absolute());
        PathBuf::from(path)
    }

    pub fn histogram_target_trigram_file(&self) -> PathBuf {
        let path = self.analytics_directory.join("histogram_target_trigram.csv");
        assert!(path.is_absolute());
        PathBuf::from(path)
    }

    pub fn histogram_target_skipgram_file(&self) -> PathBuf {
        let path = self.analytics_directory.join("histogram_target_skipgram.csv");
        assert!(path.is_absolute());
        PathBuf::from(path)
    }
    ...
    pub fn histogram_line_unigram_file(&self) -> PathBuf {
        let path = self.analytics_directory.join("histogram_line_unigram.csv");
        assert!(path.is_absolute());
        PathBuf::from(path)
    }

    pub fn histogram_line_bigram_file(&self) -> PathBuf {
        let path = self.analytics_directory.join("histogram_line_bigram.csv");
        assert!(path.is_absolute());
        PathBuf::from(path)
    }

    pub fn histogram_line_trigram_file(&self) -> PathBuf {
        let path = self.analytics_directory.join("histogram_line_trigram.csv");
        assert!(path.is_absolute());
        PathBuf::from(path)
    }

    pub fn histogram_line_skipgram_file(&self) -> PathBuf {
        let path = self.analytics_directory.join("histogram_line_skipgram.csv");
        assert!(path.is_absolute());
        PathBuf::from(path)
    }
}
