...
pub trait LodaCppCheck {
    ...
    fn perform_check2(&self, loda_program_path: &Path, time_limit: Duration) -> Result<LodaCppCheckResult, Box<dyn Error>>;
    fn perform_check_and_save_output2(&self, loda_program_path: &Path, time_limit: Duration, save_output_to_path: &Path) -> Result<LodaCppCheckResult, Box<dyn Error>>;
}
...
impl LodaCppCheck for LodaCpp {
    ...
    fn perform_check2(&self, loda_program_path: &Path, time_limit: Duration) -> Result<LodaCppCheckResult, Box<dyn Error>> {
        lodacpp_perform_check_impl_subprocess(&self, loda_program_path, time_limit, None)
    }

    fn perform_check_and_save_output2(&self, loda_program_path: &Path, time_limit: Duration, save_output_to_path: &Path) -> Result<LodaCppCheckResult, Box<dyn Error>> {
        lodacpp_perform_check_impl_subprocess(&self, loda_program_path, time_limit, Some(save_output_to_path))
    }
}
...
fn lodacpp_perform_check_impl_subprocess(
    ...
    let x = Exec::cmd(loda_cpp.loda_cpp_executable()).stream_stdout().unwrap();
